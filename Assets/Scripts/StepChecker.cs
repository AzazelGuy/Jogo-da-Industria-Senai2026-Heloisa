...
{
    [Tooltip("ID da peça cadastrado na SOPieceData ou myID do ComponenteBase.")]
...
public class StepChecker : MonoBehaviour
{
    ...
    [SerializeField] private List<ComponenteBase> pecasColocadas = new List<ComponenteBase>();

    // Eventos para atualização de UI / Lógica de Jogo
    public event Action<ComponenteBase> OnPiecePlaced;
    public static event Action OnTudoFeito;

    public IReadOnlyList<ComponenteBase> PecasColocadas => pecasColocadas;
    ...
    /// </summary>
    public void RegisterPiecePlaced(ComponenteBase peca)
    {
    ...
    public bool IsComputerFullyAssembled()
    {
        ...
        if (pecasRequeridas != null && pecasRequeridas.Count > 0)
        {
            foreach (var req in pecasRequeridas)
            {
                ...
                bool encontradaEInstalada = pecasColocadas.Exists(p => p != null && p.myID == req.pieceID && p.IsPlaced);
        ...
        // 2. Fallback: Se a lista estiver vazia, verifica se TODAS as peças na cena foram colocadas
        ComponenteBase[] todasAsPecas = FindObjectsByType<ComponenteBase>(FindObjectsSortMode.None);
        if (todasAsPecas.Length == 0) return false;

        foreach (ComponenteBase peca in todasAsPecas)
        {
            if (!peca.IsPlaced)
            {
                return false;
